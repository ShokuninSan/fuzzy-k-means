...
private[fuzzy] case class KMeansModel(val c: Int, val m: Int, centroids: DenseMatrix[Double], u: DenseMatrix[Double]) extends KMeansKernel {
...
case class KMeans(val c: Int, val m: Int) extends KMeansKernel {
  ...
  def fit(data: DenseMatrix[Double], errorThreshold: Double =  0.005, maxIterations: Int = 1000): KMeansModel =
    run(data, errorThreshold = errorThreshold, maxIterations = maxIterations) match {
      case (centroids, memberships) => KMeansModel(c, m, centroids, memberships)
    }
